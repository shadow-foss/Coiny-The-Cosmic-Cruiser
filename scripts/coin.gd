...
signal coin_collected #creates a signal that coin has been collectted
...
func _on_body_entered(body):
	...
	emit_signal("coin_collected") #sends the signal which we created usedto assign score in game scene

func flash(): #creating a new func called falsh
	sprite.material.set_shader_parameter("flash_modifier", 0.45) #increas the value makes it more opaque
